...
element["marker1"] = createMarker(593.33087158203, -1250.1072998047, 17.321031188965, "cylinder", 1.0, 0, 255, 0, 150) -- Draussen > Drin

element["marker2"] = createMarker(367.3508605957, 162.38145446777, 1024.8890625, "cylinder", 1.0, 0, 255, 0, 150) -- Drin > Draussen
setElementDimension(element["marker2"], dim)
setElementInterior(element["marker2"], 3)

element["marker3"] = createMarker(351.80215454102, 162.08778381348, 1024.8, "cylinder", 1.0, 0, 255, 0, 150) -- Drin > Tresor
setElementDimension(element["marker3"], dim)
setElementInterior(element["marker3"], 3)

element["marker4"] = createMarker(2147.7160644531, 1603.7889404297, 1005.2677246094, "cylinder", 1.0, 0, 255, 0, 150) -- Tresor > Drin
...
addEventHandler("onMarkerHit", element["marker1"], function(hitElement)
	if(getElementType(hitElement) == "player") and (isPedInVehicle(hitElement) == false) then 
		setElementData(hitElement, "inBank1", true)
...
addEventHandler("onMarkerHit", element["marker2"], function(hitElement)
	if(getElementType(hitElement) == "player") and (isPedInVehicle(hitElement) == false) then 
		if getElementData(hitElement, "inBank1") == true then
			setElementData(hitElement, "inBank1", false)
	...
	if(getElementType(hitElement) == "player") and (isPedInVehicle(hitElement) == false) then 
		if getElementData(hitElement, "inBank1") == true then
			if(isPedDead(ped[1])) and (isPedDead(ped[2])) then
				setElementData(hitElement, "inBank2", true)
...
addEventHandler("onMarkerHit", element["marker4"], function(hitElement)
	if(getElementType(hitElement) == "player") and (isPedInVehicle(hitElement) == false) then 
		if getElementData(hitElement, "inBank2") == true then
			setElementData(hitElement, "inBank2", false)
			setElementInterior(hitElement, 3)
			setElementDimension(hitElement, dim)
		...
		addEventHandler("onMarkerHit", m, function(hitElement)
			if(getElementType(hitElement) == "player") then
				...
				setTimer(function()
					...
					vioSetElementData(hitElement, "money", tonumber(vioGetElementData(hitElement, "money"))+geld)
...
addCommandHandler("robbank", function(thePlayer)
	if(doing == false) and (isElementWithinMarker(thePlayer, element["ueberfallmarker"])) --[[and (#getElementsWithinColShape(element["tresorcol"], "player") > 1) ]]then
		doing = true
		doingbankrob[thePlayer] = true
		erbeutet[thePlayer] = 0
				...
				setTimer(function()
					...
					setTimer(function()
						for index, player in pairs(getElementsWithinColShape(element["tresorcol"], "player")) do
							...
							setTimer(function()
							for index, player in pairs(getElementsWithinColShape(element["tresorcol"], "player")) do
								outputChatBox("Die Bombe explodiert in 10 Sekunden!", player, 255, 255, 0)
							end
						end, 60000, 1)
					end, 60000, 1)
				end, 60000, 1)
		...
		setTimer(function()
			...
			setTimer(detonateBomb, 300000, 1, thePlayer) --[[5*60*1000]]
			--setTimer(resetBank, 90*60*1000, 1) -- 1.5 stunden: 90*60*1000
			restimer = setTimer(resetBank, 120*60*1000, 1)
		end, 10000, 1)
	else
		outputChatBox("Es kann nur jede 1.5 Stunden ein Bankrob gestartet werden oder die Bank wird berreits Ueberfallen.", thePlayer, 200, 0, 0)
	end
end)
...
addEventHandler("onPlayerWasted", getRootElement(), function()
	if(doingbankrob[source] == true) then
		if(erbeutet[source]) then
			...
			vioSetElementData(source, "money", tonumber(vioGetElementData(source, "money"))-geld)
...
addEventHandler("onPlayerQuit", getRootElement(), function(reason)
	if (reason ~= "Kicked") and (reason ~= "Timed Out") then
		if(doingbankrob[source] == true) then
		--if (getElementDimension(source) == 6354 and getElementData(source, "fraktion") ~= 1 and getElementData(source, "fraktion") ~= 8) then
			--if(erbeutet[source]) then
				doingbankrob[source] = false
				local geld = erbeutet[source]
				if not geld then
				...
				vioSetElementData(source, "money", tonumber(vioGetElementData(source, "money"))-geld)
				timebanPlayer ( getPlayerName(source), tonumber(1), "System", "Offlineflucht beim Bankueberfall" )
			--end
		end
	end
end)

function resetTheBank(player)
if vioGetElementData ( player, "adminlvl" ) >= 4 then
resetBank()
killTimer(restimer)
end
end
addCommandHandler("resetbank", resetTheBank)